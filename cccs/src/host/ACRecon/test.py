...
parser.add_argument('-p', '--pretrain', default = './checkpoints/LEVIR_k3_32_64_16384/iter_220600.pth.tar',
        help='load pretrain model')
parser.add_argument('-t', '--test', default='kodak',
        help='test dataset')
parser.add_argument('--config', dest='config', default = './examples/example/config_low_16384.json', required=False,
...
parser.add_argument('--val', dest='val_path', default='./test256/', help='the path of validation dataset')
...
def test(step):
    with torch.no_grad():
        ...
        sumMsssimDB = 0
        cnt = 0
        for batch_idx, (input, filename) in enumerate(test_loader):
            # ---------------------------------------------------------------------------
            print(input.shape)
            a = input.cpu().detach().numpy().reshape(3, 256,256)
            for i in range(a.shape[0]):
                file1 = './data/input_'+str(i)+'.csv'
                b = np.array(a[i,:,:]).reshape(256,256)
            ...
            logger.info("Num: {}, name: {}, Bpp:{:.6f}, Bpp_z: {:.6f}, PSNR:{:.6f}, MS-SSIM:{:.6f}, MS-SSIM-DB:{:.6f}".format(cnt, filename, bpp, bpp_z, psnr, msssim, msssimDB))

            fakepath = "outputs/recon/recon.png"

            torchvision.utils.save_image(clipped_recon_image, fakepath, normalize=False)
...
if __name__ == "__main__":
    ...
    model = LoadImageCompressor(out_channel_N, out_channel_M)
    if args.pretrain != '':
    ...
    '''if args.test == 'kodak':
        ...
        exit(-1)'''
    test_dataset = TestKodakDataset(data_dir=args.val_path)
    print(len(TestKodakDataset(data_dir=args.val_path)))

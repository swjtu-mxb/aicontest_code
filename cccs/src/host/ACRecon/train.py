...
parser.add_argument('-n', '--name', default='LEVIR_k3_32_64_16384', help='experiment name')
parser.add_argument('-p', '--pretrain', default='', help='load pretrain model')
parser.add_argument('--test', action='store_true')
parser.add_argument('--config', dest='config', default = './examples/example/config_low_16384.json' , help='hyperparameter in json format')
parser.add_argument('--seed', default=234, type=int, help='seed for random functions, and network initialization')
parser.add_argument('--train', dest='train', default = '../LEVIR-CD-CROP/train/B/' , help='the path of training dataset')
parser.add_argument('--val', dest='val', default = '../LEVIR-CD-CROP/val/B/', help='the path of validation dataset')
...
def train(epoch, global_step):
    ...
    for batch_idx, input in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        #rd_loss.backward()
        rd_loss.backward(torch.ones_like(distortion))
        ...
        if (global_step % save_model_freq) == 0 and (epoch % save_model_epoch) == 0:

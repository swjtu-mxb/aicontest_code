...
class Maskb_resblock(nn.Module):
    ...
    def forward(self, x):
        x1 = self.conv2(f.relu(self.conv1(x), inplace=True))
        out = x+x1
        return out
...
class Weighted_Gaussian(nn.Module):
    def __init__(self, M):
        ...
        self.conv2 = nn.Sequential(nn.Conv3d(25, 48, 1, 1, 0), nn.ReLU(inplace=True), nn.Conv3d(48, 96, 1, 1, 0), nn.ReLU(inplace=True),
    ...
    def forward(self, x, hyper):
        x = torch.unsqueeze(x.clone(), dim=1)

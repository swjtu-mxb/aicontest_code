...
class Synthesis_net(nn.Module):
    ...
    def __init__(self, out_channel_N=192, out_channel_M=320):
        ...
        torch.nn.init.xavier_normal_(self.deconv1.weight.data, (math.sqrt(2 * 1 * (2 * out_channel_M + out_channel_N) / (2 * out_channel_M + 2 * out_channel_M))))
        ...
        self.deconv4 = nn.ConvTranspose2d(out_channel_N, 3, 3, stride=2, padding=1, output_padding=1)
        torch.nn.init.xavier_normal_(self.deconv4.weight.data, (math.sqrt(2 * 1 * (out_channel_N + 1) / (out_channel_N + out_channel_N))))

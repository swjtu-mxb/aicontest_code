...
class Distribution_for_entropy2(nn.Module):
    ...
    def forward(self, x, p_dec):
        ...
        #print(p_dec.shape)
        prob0=  torch.chunk(p_dec, 9, dim=1)[0].squeeze(1)
        mean0 = torch.chunk(p_dec, 9, dim=1)[1].squeeze(1)
        scale0 = torch.chunk(p_dec, 9, dim=1)[2].squeeze(1)
        prob1 = torch.chunk(p_dec, 9, dim=1)[3].squeeze(1)
        mean1 = torch.chunk(p_dec, 9, dim=1)[4].squeeze(1)
        scale1 = torch.chunk(p_dec, 9, dim=1)[5].squeeze(1)
        prob2 = torch.chunk(p_dec, 9, dim=1)[6].squeeze(1)
        mean2 = torch.chunk(p_dec, 9, dim=1)[7].squeeze(1)
        scale2 = torch.chunk(p_dec, 9, dim=1)[8].squeeze(1)
        ...
        probs = torch.stack([prob0.clone(), prob1.clone(), prob2.clone()], dim=-1)
        #print(probs.shape)
        probs = f.softmax(probs.clone(), dim=-1)
